...
- **程式內代理** — Modern 與 SmallTool 可共用 HTTP／HTTPS／SOCKS Proxy，不必修改 Windows 全域代理；網站解析、縮圖、m3u8、影片分片與 SupJav 直連下載皆會套用
...
4. **設定分頁** — 調整速度限制、儲存位置、畫質偏好；需代理時填入 `127.0.0.1:7890` 或完整 HTTP／SOCKS URL 後儲存
5. **開啟資料夾** — 點擊「開啟資料夾」按鈕直接查看下載的影片
6. **取消 / 全部取消** — 可隨時中止下載任務

## 更新紀錄

- **v2.5.29** — 🌐 **新增程式內 Proxy 設定**（#32）：Modern 與 SmallTool 共用代理設定，支援 HTTP／HTTPS／SOCKS4／SOCKS5；網站解析、縮圖、m3u8、AES 金鑰、影片分片、SupJav 直連 MP4 與 App 更新流量均會套用，不會修改 Windows 全域代理，儲存後立即生效。
- **v2.5.27** — 🌐 **三站跨分類／跨來源番號去重**：SmallTool 將版本優先套用到 JableTV、MissAV、SupJav，提供中文字幕、無碼／流出、一般版、英文字幕與馬賽克減弱五種選項（預設中文字幕）。同一番號出現在多個分類、不同頁面或不同來源時只保留符合偏好的版本；已成功下載的偏好版本不會因換站重抓，偏好改變時則可下載新的優先版本。無法由 URL／標題確認番號時只按完全相同 URL 去重，不猜測合併。
